...
**Can I download the Overview or share it with a teammate? (Phase 15)**
Yes. The Insights → Overview toolbar has three buttons next to *Run now* and *Refresh*: **Export CSV**, **Export JSON**, and **Copy share link**. *Export CSV* and *Export JSON* download the current cards **plus** the recent history transitions for the same window into a single file — `axion-insights-overview-YYYYMMDD-HHMMSS.csv` / `.json`. The CSV has one fixed header row (section / category / severity / state / title / summary / why_it_matters / recommended_action / affected_holdings / confidence / first_seen_at / last_seen_at / notified_at / deep_link_label / deep_link_surface / deep_link_subtab / source_type). *Copy share link* writes a URL whose hash carries the full Overview filter set; opening it on another machine restores the same slice. The export and the hash never carry API keys, OAuth or Telegram tokens, AI prompt bodies, uploaded document content, or `.env` contents — those are scrubbed structurally (only customer-safe model fields are emitted) and verified by a privacy gate that scans every export response for forbidden substrings.
